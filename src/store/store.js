...
export const store = new Vuex.Store({
    ...
    getters: {
        ...
        discountTotalAmount: (state) => {
            let harryCount = state.cart.filter((obj) => obj.promotion === true);
            if( harryCount.length > 1)
            {
                let percent = (harryCount.length-1)/10;
                let minQualityDis = harryCount.reduce((prev, curr)=>{
                    return prev.quantity < curr.quantity ? prev : curr;
                }).quantity;
                return harryCount.reduce((total, product) => {
                    return total + (product.price * minQualityDis);
                }, 0)*percent;
            }
            else
            {
